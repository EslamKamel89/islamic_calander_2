...
class MoonPhaseCubit extends Cubit<MoonPhaseState> {
  ...
  Future getMoonPhase(int year, MoonPhaseEnum moonPhase) async {
    ...
    emit(state.copyWith(getMoonPhaseState: ResponseState.loading, selectedYear: year));
    final result = await dateInfoRepo.getMoonInfoMonth(year, moonPhase);
    return result.fold(
      (Failure failure) {
        pr(failure, t);
        emit(state.copyWith(getMoonPhaseState: ResponseState.failure, moonPhasesInfo: []));
      },
      (List<MoonInfoModel> models) {
        ...
        emit(
          state.copyWith(
            ...
            getMoonPhaseState: ResponseState.success,
          ),
        );
      },
    );
